...
from typing import Union, Optional, Tuple, Mapping, Sequence, Dict
import math
from movinet.config import (
    BLOCK_SPECS, StemSpec, HeadSpec, TensorMap, MovinetBlockSpec, BlockSpec
)
from movinet.layers.stem import Stem

from movinet.blocks.movinet_block import MovinetBlock
from movinet.layers.head import Head


class Movinet(keras.Model):
    ...
    def __init__(
        ...
        conv_type: str = "3d",
        se_type: str = "3d",
        input_specs: Optional[keras.layers.InputSpec] = None,
        ...
        **kwargs,
        ...
        if input_specs is None:
            input_specs = keras.layers.InputSpec(shape=[None, None, None, None, 3])
    ...
    def _build_network(
        ...
        state_specs: Optional[Mapping[str, keras.layers.InputSpec]] = None,
        ...
        image_input = keras.Input(shape=input_specs.shape[1:], name="inputs")

        states = {
            name: keras.Input(shape=spec.shape[1:], dtype=spec.dtype, name=name)
        ...
        for block_idx, block in enumerate(self._block_specs):
            if isinstance(block, StemSpec):
                ...
                params = list(
                    zip(block.expand_filters, block.kernel_sizes, block.strides)
                )
        ...
        for block_idx, block in enumerate(block_specs):
            if isinstance(block, StemSpec):
                ...
                for layer_idx, layer in enumerate(params):
                    ...
                    # before the buffer.
                    if tuple(strides[1:3]) != (1, 1) and self._conv_type in [
                        "2plus1d",
                        "3d_2plus1d",
                    ]:
                        num_downsamples += 1
                    ...
                    # after the buffer.
                    if tuple(strides[1:3]) != (1, 1) and self._conv_type not in [
                        "2plus1d",
                        "3d_2plus1d",
                    ]:
                        num_downsamples += 1
    ...
    ) -> Dict[str, keras.layers.InputSpec]:
