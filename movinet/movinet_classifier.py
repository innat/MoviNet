...
from typing import Union, Optional, Tuple, Mapping, Sequence, Dict, Any

from movinet.layers.classifier_head import ClassifierHead

class MovinetClassifier(keras.Model):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not input_specs:
            input_specs = {
                "image": keras.layers.InputSpec(shape=[None, None, None, None, 3])
            }
    ...
    def _build_backbone(
        self,
        backbone: keras.Model,
        input_specs: Mapping[str, keras.layers.InputSpec],
        ...
        image = keras.Input(shape=input_specs["image"].shape[1:], name="image")
    ...
    def _build_network(
        ...
        Mapping[str, keras.Input],
        Union[
            Tuple[
                Mapping[  # pytype: disable=invalid-annotation  # typed-keras
                    str, tf.Tensor
                ],
        ...
        if config["input_specs"]:
            for name in config["input_specs"]:
                if isinstance(config["input_specs"][name], dict):
                    config["input_specs"][name] = keras.layers.deserialize(
